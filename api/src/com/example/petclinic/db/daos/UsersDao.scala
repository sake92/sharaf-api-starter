...
class UsersDao {
  ...
  def findAll(): DbAction[Seq[UsersRow]] = sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS".readRows()
  def findAllWhere(whereQuery: Query): DbAction[Seq[UsersRow]] =
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE $whereQuery".readRows()
  def findWhere(whereQuery: Query): DbAction[UsersRow] =
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE $whereQuery".readRow()
  def findWhereOpt(whereQuery: Query): DbAction[Option[UsersRow]] =
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE $whereQuery".readRowOpt()
  def findById(id: UsersRow.PK): DbAction[UsersRow] =
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE USERNAME = $id".readRow()
  def findByIdOpt(id: UsersRow.PK): DbAction[Option[UsersRow]] =
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE USERNAME = $id".readRowOpt()
  def findByIds(ids: Set[UsersRow.PK]): DbAction[Seq[UsersRow]] = {
    val idsExpr = ids.map(id => sql"${id}").reduce(_ ++ (sql",") ++ _)
    sql"SELECT USERNAME, PASSWORD, ENABLED, FIRST_NAME FROM PUBLIC.USERS WHERE USERNAME IN ($idsExpr)".readRows()
  }
  def insert(row: UsersRow): DbAction[Int] = sql"""INSERT INTO PUBLIC.USERS(USERNAME, PASSWORD, ENABLED, FIRST_NAME) 
VALUES (
      ${row.USERNAME},${row.PASSWORD},${row.ENABLED},${row.FIRST_NAME}    
)""".insert()
  def updateById(row: UsersRow): DbAction[Int] = sql"""UPDATE PUBLIC.USERS 
SET PASSWORD = ${row.PASSWORD}, ENABLED = ${row.ENABLED}, FIRST_NAME = ${row.FIRST_NAME}
