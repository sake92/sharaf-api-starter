...
class FlywaySchemaHistoryDao {
  ...
  def insert(row: FlywaySchemaHistoryRow): DbAction[FlywaySchemaHistoryRow] =
    ...
    RETURNING installed_rank, version, description, type, script, checksum, installed_by, installed_on, execution_time, success"""
      .insertReturningRow()
